...
population_size=(96) # 50 100 200 400)
max_oracle_call=(-1 ) # 200 500 1000 2000)
iteration=(16) # -1 -1 -1 -1)
...
# done
python3 -c "import time; time.sleep(60 * 30)" 
for i in $(seq 0 $((length - 1))); do
    pop_size=${population_size[$i]}
    max_call=${max_oracle_call[$i]}
    iter=${iteration[$i]}
    ...
    # Perform your operation here
    python run.py molleo --mol_lm Llama3 --dataset Syn-3bfo --population_size $pop_size --iteration $iter --max_oracle_calls $max_call --seed 1 2 3 --oracles fitness
